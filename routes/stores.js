...
router.get("/search/filter", (req, res) => {
    ...
    const sql = "SELECT * FROM store WHERE store_name LIKE ? AND city LIKE ? ORDER BY store_id";
...
router.post("/", (req, res) => {
    const { store_id, store_name, city, address, contact_number } = req.body;
    if (!store_id || !store_name)
        return res.status(400).json({ error: "store_id and store_name are required" });

    const sql = "INSERT INTO store (store_id, store_name, city, address, contact_number) VALUES (?, ?, ?, ?, ?)";
    db.query(sql, [store_id, store_name, city || null, address || null, contact_number || null], (err) => {
        if (err) {
...
router.put("/:id", (req, res) => {
    const { store_name, city, address, contact_number } = req.body;
    if (!store_name)
        return res.status(400).json({ error: "store_name is required" });

    const sql = "UPDATE store SET store_name = ?, city = ?, address = ?, contact_number = ? WHERE store_id = ?";
    db.query(sql, [store_name, city || null, address || null, contact_number || null, req.params.id], (err, result) => {
